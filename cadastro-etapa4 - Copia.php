<?
	...
	if ($_POST){
		...
		if(isset($_POST['tipoPagamentoBonus'])){
			...
			$login				= $_POST['login'];
			$pontosTotais		= $_POST['pontosTotais'];
			$pontosConsumidos	= $_POST['pontosConsumidos'];
			$idTelexFree		= $_POST['idTelexFree'];
			$emailTelex			= $_POST['emailTelex'];
			...
			$sql = 	"UPDATE cabine SET idStatus = ".STATUS_OCUPADO."
					WHERE idCabine in (
						SELECT idCabine 
						FROM vendacabine
				...
				<form id="frmDadosTelexFree" name="frmDadosTelexFree" method="post" action="" >	
					...
					<div id="boxBonus">
						<fieldset class='cadastro'>
							<legend>Login do TelexFree</legend>
								<p>Entre com seus dados do TelexFree.</p>
								...
								<input type="text" class="itemForm2" name="emailTelex" id="emailTelex" placeholder="Email" maxlength="70" /><br />
								<input type="text" class="itemForm2" name="login" id="login" placeholder="Login" maxlength="70" /><br />
								<input type="password" class="itemForm2" name="senha" id="senha" placeholder="Senha" maxlength="70" /><br />
								<a href="javascript:;" id="bt-login" class="bt-login btnInterno comMargem">Login</a>
					...
					<div id="quantBonus">
						<fieldset class='cadastro'>
							<legend>Seus bônus no TelexFree</legend>
							
							<div id="MsgErroValidaAceite" class='MsgError'>
								Os seguintes erros foram encontrados:<br />
								- Os campos em vermelho são de preenchimento obrigatório ou contém um valor inválido.
							</div>	
							
							<img src="inc/img/telexfree.jpg" id="imgQuantBonus" alt="TelexFree" title="TelexFree" />
							<div id="contQuantBonus">
								<h4 id="bonusTitulo">Parabéns!</h4>
								<p>Você tem <span id="quantidadeBonus">4000 pontos</span> no TelexFree.
								<br />
								Esses pontos são suficientes para esse cruzeiro!</p>									
								<input type="hidden" id="pontosTotais" name="pontosTotais" value="4000" />
								<input type="hidden" id="idTelexFree" name="idTelexFree" value="26780" />
								<input type="hidden" id="pontosConsumidos" name="pontosConsumidos" value="2000" />
								<p>Para concluir aceite o termo e clique no botão 'Finalizar' ou selecione outra forma de pagamento.</p>
								...
								<a href="javascript:;" id="bt-bonus" class="bt-login btnInterno comMargem">Finalizar</a>
							</div>
			...
			$(document).ready(function(){	
				...
				$('#bt-bonus').click(function(){
					...
					if (!$("#aceitoTermoBonus").is(':checked')){
						...
						$("#MsgErroValidaAceite").attr("style", "display:block;");
					}else{
